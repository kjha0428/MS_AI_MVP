...
### 🤖 시나리오2. AI 쿼리 생성 예시
#### 1.	사업자별 정산 조회
o	입력: "2024년 1월 A통신사 포트인 정산 금액 알려줘"
o	결과: 조건(월/사업자/번호이동타입)에 따른 집계 쿼리 자동 생성

#### 2.	개별 번호 정산 검증
o	입력: "010-1234-5678 번호 정산 내역 확인해줘"
o	결과: 테이블 JOIN 하여 정산금액 차액 계산 쿼리 생성

#### 3.	이상 징후 분석
o	입력: "정산 금액이 급증한 사업자 찾아줘"
o	결과: 월별 증가율 분석 및 임계값 초과 사업자 추출 쿼리 생성
